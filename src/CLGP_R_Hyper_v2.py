...
class CLGP_R():
    ...
    def _create_metrics(self):
        ...
        self.acc_test_pred, self.test_pred = self._Compute_pred_acc(self.tilde_X, y_test, n_incomplete=args.n_incomplete)
    ...
    def _Compute_pred_acc(self, X, Y, n_incomplete=0):
        ...
        # print(boolean_mat)
        return tf.reduce_mean(tf.cast(boolean_mat, tf.float32)), pred_mat
    ...
    def Test(self, Y_test, n_rs = 1, display_step=5, testing_epochs=1000, model_path='./model/model_tem.ckpt', verbose=False, savefig=False):
        # create folder for training figures
        try:
        ...
        for indx in range(n_rs):
            ...
            for epoch in range(self.testing_epochs):
                ...
                if epoch % display_step == 0:
                    # print training information
                    self.est_tilde_X, self.est_ell_test, self.est_test_pred, self.est_acc_test_pred, self.est_ell_test_pred, self.est_lp_test_pred, self.est_Z = self.sess.run([self.tilde_X, self.ell_test, self.test_pred, self.acc_test_pred, self.ell_test_pred, self.lp_test_pred, self.Z])
                    logging.info("Epoch: {}".format(epoch+1))
            ...
            est_test_pred_list.append(self.est_test_pred)
        ...
        self.est_test_pred_list = est_test_pred_list
...
if __name__=="__main__":
    ...
    with open("pars_test.pickle", "wb") as file:
        pickle.dump([clgp_r.est_tilde_X, clgp_r.est_Z, clgp_r.est_test_pred_list, clgp_r.est_acc_test_pred_list, clgp_r.est_lp_test_pred_list, clgp_r.est_tilde_X_list], file)
